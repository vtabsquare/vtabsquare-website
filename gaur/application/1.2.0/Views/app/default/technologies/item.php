    ...
    <title><?= hentities($technology['title']) ?> - <?= config('Config\App')->siteTitle ?></title>

    <!-- meta for search engines -->
    <link rel="canonical" href="<?= config('Config\App')->baseURL ?>technology/<?= hentities($technology['slug']) ?>">
    <meta name="robots" content="follow, index">

    <?php if ($technology['mdesc']): ?>
    <meta name="description" content="<?= hentities($technology['mdesc']) ?>">
    <?php endif; ?>

    <?php if ($technology['mkeywords']): ?>
    <meta name="keywords" content="<?= hentities($technology['mkeywords']) ?>">
    <?php endif; ?>

    <?= view('app/default/technologies/social_metadata') ?>
    ...
    <div class="container py-5">
        <div class="row">
            <div class="col-lg-12">
                ...
                <a data-lity href="images/technologies/<?= $technology['image'] ?>">
                    <img class="img-fluid d-block mx-auto mb-3" src="images/technologies/<?= $technology['image'] ?>" alt="" style="width: 600px; max-width: 100%" />
                </a>
                <?php endif; ?>

                <?php if ($technology['video']): ?>
                <iframe width="560" height="315" src="https://www.youtube.com/embed/<?= $technology['video'] ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen style="max-width: 100%; margin: 0 auto;width: 560px;display: block;"></iframe>
                <br>
                <?php endif; ?>

                <div><?= $technology['info'] ?></div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-xl-4 col-md-6">
                <div class="technology-card p-0">
                    ...
                    <?php endif; ?>
                    <div class="p-4">
                        <h3 class="box-title"><a href="technology/<?= hentities($item['slug']) ?>"><?= hentities($item['title']) ?></a></h3>
                        <p class="box-text" style="height: 4.5em; overflow: hidden"><?= implode(' ', array_slice(explode(' ', strip_tags($item['info'])), 0, 100)) ?></p>
                        <a href="technology/<?= hentities($item['slug']) ?>" class="box-btn"><i class="fas fa-arrow-up-right"></i></a>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
