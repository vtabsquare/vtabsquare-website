    ...
    <title><?= hentities($casestudy['title']) ?> - <?= config('Config\App')->siteTitle ?></title>

    <!-- meta for search engines -->
    <link rel="canonical" href="<?= config('Config\App')->baseURL ?>case-studies/<?= hentities($casestudy['slug']) ?>">
    <meta name="robots" content="follow, index">

    <?php if ($casestudy['mdesc']): ?>
    <meta name="description" content="<?= hentities($casestudy['mdesc']) ?>">
    <?php endif; ?>

    <?php if ($casestudy['mkeywords']): ?>
    <meta name="keywords" content="<?= hentities($casestudy['mkeywords']) ?>">
    <?php endif; ?>

    <?= view('app/default/casestudy/social_metadata') ?>
    <?= view('app/default/common/css') ?>
    <?= view('app/default/common/head_bottom') ?>
    <?= view('app/default/common/menu', ['pageName' => hentities($casestudy['title'])]) ?>
    ...
    <div class="container py-5">
        <div class="row">
            <div class="col-lg-12">
                ...
                <a data-lity href="images/casestudies/<?= $casestudy['image'] ?>">
                    <img class="img-fluid d-block mx-auto mb-3" src="images/casestudies/<?= $casestudy['image'] ?>" alt="" style="width: 600px; max-width: 100%" />
                </a>
                <?php endif; ?>

                <?php if ($casestudy['video']): ?>
                <iframe width="560" height="315" src="https://www.youtube.com/embed/<?= $casestudy['video'] ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen style="max-width: 100%; margin: 0 auto;width: 560px;display: block;"></iframe>
                <br>
                <?php endif; ?>

                <div><?= $casestudy['info'] ?></div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <h2 class="h4 pb-4">Related <?= hentities($casestudy['title']) ?></h2>
            </div>

<?php
$hasOtherCasestudies = false;
foreach ($casestudies as $item) {
    if ($item['id'] != $currentCasestudyId) {
        $hasOtherCasestudies = true;
        break;
    }
}
?>

<?php if (!$hasOtherCasestudies): ?>
    <p>No related Case Studies found.</p>
<?php endif; ?>
    
            <?php foreach ($casestudies as $item): ?>
            
                    <?php if (isset($currentCasestudyId) && $item['id'] == $currentCasestudyId) continue; ?>
            ...
            <div class="col-xl-4 col-md-6">
                <div class="service-card p-0">
                    <?php if ($item['image']): ?>
                    <div class="box-icon mb-0 w-auto h-auto"><img src="images/casestudies/thumb/<?= $item['image'] ?>" alt /></div>
                    <?php endif; ?>
                    <div class="p-4">
                        ...
                        <a href="case-studies/<?= hentities($item['slug']) ?>" class="box-btn"><i class="fas fa-arrow-up-right"></i></a>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
