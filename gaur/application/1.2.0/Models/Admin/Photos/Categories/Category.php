...
use App\Data\Photos\Categories\CategorySchema;
use Gaur\Model;
...
class Category extends Model
{
    ...
    public function add(array $rdata): int
    {
        ...
        $qry = 'INSERT INTO `' . $this->db->prefixTable('photos_categories') . '`(`title`, `slug`, `pid`, `total`, `status`)
                VALUES ('
    ...
    {
        $qry = 'UPDATE `' . $this->db->prefixTable('photos_categories') . '`
    ...
    {
        $qry = 'SELECT `id`
                FROM `' . $this->db->prefixTable('photos_categories') . '`
                WHERE `id` = ' . $id;
