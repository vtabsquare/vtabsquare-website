...
use App\Data\Casestudy\Categories\CategoryParentSchema;
use Gaur\Model;
...
class CategoryParent extends Model
{
    ...
    public function add(int $cid, array $rdata): void
    {
        ...
        $qry = 'INSERT INTO `' . $this->db->prefixTable('casestudy_category_parents') . '`(`cid`, `pid`, `rid`)
                VALUES ';
    ...
    {
        $qry = 'SELECT `cid`
                FROM `' . $this->db->prefixTable('casestudy_category_parents') . '`
                WHERE `pid` = ' . $pid;
    ...
    {
        $qry = 'DELETE FROM `' . $this->db->prefixTable('casestudy_category_parents') . '`
                WHERE `cid` = ' . $cid;
