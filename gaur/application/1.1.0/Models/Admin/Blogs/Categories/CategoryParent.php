...
use App\Data\Blogs\Categories\CategoryParentSchema;
use Gaur\Model;
...
class CategoryParent extends Model
{
    ...
    public function add(int $cid, array $rdata): void
    {
        ...
        $qry = 'INSERT INTO `' . $this->db->prefixTable('blogs_category_parents') . '`(`cid`, `pid`, `rid`)
                VALUES ';
    ...
    {
        $qry = 'SELECT `cid`
                FROM `' . $this->db->prefixTable('blogs_category_parents') . '`
                WHERE `pid` = ' . $pid;
    ...
    {
        $qry = 'DELETE FROM `' . $this->db->prefixTable('blogs_category_parents') . '`
                WHERE `cid` = ' . $cid;
