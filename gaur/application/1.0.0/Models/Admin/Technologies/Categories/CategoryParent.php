...
use App\Data\Technologies\Categories\CategoryParentSchema;
use Gaur\Model;
...
class CategoryParent extends Model
{
    ...
    public function add(int $cid, array $rdata): void
    {
        ...
        $qry = 'INSERT INTO `' . $this->db->prefixTable('technologies_category_parents') . '`(`cid`, `pid`, `rid`)
                VALUES ';
    ...
    {
        $qry = 'DELETE FROM `' . $this->db->prefixTable('technologies_category_parents') . '`
                WHERE `cid` = ' . $cid;
